...
**当前目标**：完成启航平台本期迭代（PMS 关联 / 操作手册 / 新一代应用地图 / 更新日志）的原型实现并推送 origin/main；新增 workbench（新一代首页）平台及「待办工作台」模块（202607 批次），进入需求规划设计阶段。
...
- **新一代首页**（待办工作台）：聚合展示各系统待办信息，快捷入口，团队管理（新增平台，202607 批次首发）
- 三平台关系：启航「分发」→ 知行「赋能」→ 聆听「反馈」→ 启航「迭代」

**当前优先级**：
1. 🔴 新一代首页·待办工作台需求规划设计：UX 研究员分析已完成，方案设计中
2. 🔵 53 应用 / 10 大分类重新规划：应用地图 app_map.html 布局持续调整（一级分类去编号、监管报送与工具平台并排）
3. 🟡 预览修复与页面简化：workbench_202607.html 占位内容简化、样式对齐
...
- [x] **待办工作台批次页**：`requirements/workbench_202607.html`（`data-platform="workbench" data-batch="202607"`），单卡片占位，UX 研究员需求分析已完成（三材料综合：方案文档 + 原型截图 + 业务流程图）
...
### 4.8 app_map 应用地图布局
- **一级分���去编号**：7 个一级分类标题去掉「一、…七、」编号（二级保留）
