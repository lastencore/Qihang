...
- [ ] **[产品设计] [启航]** 53 应用 / 10 大分类重新规划 + UI 设计：用户正在推进中，涉及合并、拆分；应用地图同步调整
- [ ] **[产品设计] [新一代首页]** 待办工作台方案设计：方案脑图+原型截图已发领导，等待反馈修改意见；最终方案文档待领导反馈后输出
...
### 4.12 app_map Banner 设计 ✅ 已定
- 左侧：标题 + 4 红色指标卡（7 大领域/17 二级/53 应用/3.2 万用户）
- 右侧：SVG 7 层金字塔 + 7 张系统卡片（左侧彩色竖条编号）+ 虚线引导线 + hover 联动
- 底部滚动提示"点击进入地图"；探索版方案归档至 `banner_v1/v2/v3_*.html`
