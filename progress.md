...
**2026-07-08（本次，已推送 origin/main）**

- [x] **应用管理「应用简介」字段**：`admin/app_management.html` 新增应用简介字段（列表列 + 新增/编辑弹窗），commit `731d4ae`
- [x] **应用地图搜索扩展**：`client/app_map.html` 搜索支持按名称 / 属主 / 分类检索，commit `731d4ae`
- [x] **index.html 覆盖客户端首页**：`index.html` 重写为新导航索引（合并卡片 + tag 分类 + PM 署名）；原 `client/client.html` 标记弃用，commit `731d4ae`
- [x] **images/ 配图**：新增 `mapping_relation.png`、`update_log_flow.jpg`，commit `731d4ae`
- [x] **清理 index_2.html**：与 index.html 内容一致后删除，commit `93ceda6`
- [x] **推送 origin/main**：`731d4ae`（feat）+ `93ceda6`（chore 删 index_2）→ 已推送；远端 tracking 已对齐
- [x] **本期迭代 PRD 草稿**：`/workspace/启航平台_产品需求说明书_202607.md`（PMS 关联 / 新一代应用地图 / 更新日志 / 操作手册 四模块，EARS 写法 + 验收标准，参考聆听/知行社历史 PRD 文风）——**未入库**（路径待用户定：docs/ vs 根目录；按 §4.6 需用户说「推送」再入库）
...
| `启航平台_应用分类.md` | 分类本地副本 |
| `启航平台_产品需求说明书_202607.md` | **本期迭代 PRD 草稿**（四模块，EARS 写法）→ 待入库推送 |
