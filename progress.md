...
**当前目标**：
- **[启航]** 本期迭代（PMS 关联 / 操作手册 / 应用地图 / 更新日志）开发中；**P1 降级方案「新一代应用清单」页已完成交付**，等终版清单后做 UI 设计
...
| 平台 | 定位 | 状态 | 关键事项 |
|------|------|------|----------|
| **启航** | 应用地图 / 员工作业门户 | 🟢 开发中 | 53应用10分类重规划推进中；**降级方案「新一代应用清单」页已交付** |
...
| 🔵 P1 | 启航 | 53应用/10大分类重新规划 + UI设计 | 等终版清单（下周一）；**降级方案应用清单页已交付**，拿到清单后约 UI 设计 |
...
**2026-08-07（本次）— 文案修正：manual_query「手册大盘清单」→「操作手册管理」**

- [x] **[产品设计] [启航] manual_query.html 面包屑文案**：「手册大盘清单」→「操作手册管理」（唯一一处，验证 0 残留）；**后续任务**：旧分类→新分类数据清洗（读 `运营支撑域系统应用列表（新一代）.xlsx` 梳理映射关系，P1 降级方案数据准备）
...
- [x] **[产品设计] [启航] 新增 `qihang/admin/app_list.html` 新一代应用清单页**：面向普通用户的应用清单（P1 降级方案先行交付，不等终版清单）；53 应用数据与 `app_detail.html` appsData 一致；表格仅 4 列（应用名称 / 应用简介 / 属主 / 操作），操作唯一项「查看应用详情」→ `../client/app_detail.html?name=`；查询区 = 关键字搜索（应用名称+属主，实时过滤）+ 属主部门下拉（动态提取）+ 重置/查询；无管理员属性（编码/可见属性/PMS/排序/管理员等）
- [x] **[产品设计] [启航] 查询区自包含 CSS 规范**：`.app-filter-*` 类（页面内联，**不依赖 antd 表单类/CDN/缓存**），视觉对齐 manual_query（白卡 padding 24px、32px 控件、聚焦蓝框 #4096ff、主按钮 #cc0000）；属主多值用 `ant-tag`，放不下的 tag **整体隐藏** + 尾部 `…` + 容器 `title` 悬浮显示全部；提示词模板已输出供未来新页面复用
- [x] **[产品设计] [启航] 管理端导航互通**：`sidebar.tpl` 新增「新一代应用清单」菜单项（6 个管理端页侧栏全生效，双向互通）；`app_detail.html` 返回按钮改指 `../admin/app_list.html`、文案「返回应用清单」
- [x] **[产品设计] [启航] 需求批次页入口**：`requirements/qihang_202606.html`「二、启航应用管理与客户端 → 客户端原型」新增「新一代应用清单」链接（已验证 200）
- [x] **名称统一**：「启航应用清单」→「新一代应用清单」（页面 title/面包屑/heading/侧栏菜单）
- [x] **验证**：playwright 全链路通过（查询区样式结构 / 搜索筛选重置 / 多属主 tag 溢出隐藏 / 改名 / 详情跳转返回 / 批次页链接可达 / 0 JS 错误）
...
**2026-07-30（本次）—— 待办状态同步与收尾**

> 用户清点待办后的状态对齐（详见对话）：

- [x] **PRD 同步完成（用户本地收口并推送）**：本期迭代 4 项改动（含「应用可见属性」必填联动）段落已并入 `docs/启航平台_“应用管理”“应用地图”“更新日志”“操作手册”_PRD.md`；原 `启航平台_新一代应用地图_PRD.md` 已删除，`启航平台_产品需求说明书_202607.md` 重命名为四模块合并版（AI 已 `git pull` 对齐到 `785ae8d`）
- [x] **外网预览同步完成（用户本地跑 `upload.bat`）**：原型已发布到 OSS/ECS 外网
- [ ] **[中心建设] 域名备案已完成**：待绑自定义域名（DNS A 记录 → 114.55.130.110，直连 ECS 无需 CNAME）+ 申请 SSL 证书 + Nginx 加 443 + 网站底部挂 ICP 备案号（用户下一步执行）
- [ ] **[新一代首页] 待办工作台方案**：继续等待领导反馈修改意见（P0）
- [ ] **[启航] 能力地图/岗位场景地图具体形式**：继续等待用户设计
- [ ] **[新一代首页] 接入范围与数据标准**：继续等待确认
- [ ] **[知行] 平台启动**：继续等待，计划下周启动产品需求与原型
...
├── qihang/                    # ★启航系统（全部端集中）
│   ├── admin/                 # 管理端原型（7 页，均 <body data-system="qihang">）
│   │   ├── app_list.html       # 新一代应用清单（面向普通用户，P1 降级方案；查询区自包含 CSS）
│   │   ├── app_management.html
...
| `启航平台_应用分类.md` | 分类本地副本 |
| `docs/启航平台_“应用管理”“应用地图”“更新日志”“操作手册”_PRD.md` | **本期迭代 PRD**（四模块合并，已入库 `docs/`）✅ |
...
### 4.14 新一代应用清单页（P1 降级方案，2026-08-06）✅ 已交付
- **定位**：面向普通用户的应用清单，降级方案先行交付（不等终版清单）；位于管理端导航体系内（`qihang/admin/app_list.html`，侧栏互通），内容为普通用户视角（无管理员属性列/操作）
- **查询区自包含 CSS 规范（重要约定）**：`.app-filter-*` 类写**页面内联**，**不依赖 antd 表单类（ant-form/ant-select/ant-input-affix-wrapper 等）/CDN/外部缓存**；视觉对齐 manual_query（白卡 padding 24px、32px 控件、边框 #d9d9d9 圆角 2px、聚焦 #4096ff、主按钮 #cc0000）；未来新建带查询区的管理端页面统一用此方案（标准提示词模板已给用户，待写入 html-prototype-guide skill）
...
- [启航] 应用分类/应用清单终版：等**其他部门下周一**给终版清单；**降级方案应用清单页已先行交付**（`qihang/admin/app_list.html`），拿到终版清单后约 UI 设计（含 P3 全景图）
- [新一代首页] 接入范围与数据标准（待确认）
